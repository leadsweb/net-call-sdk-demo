...
import TencentNetCall, { CalloutState } from 'net-call-sdk';
import type { InputRef } from 'antd';
import type { FormInstance } from 'antd/es/form';
import axios from 'axios';
import Caller, { CallerState, CallUser } from '../../components/Caller';
...
const clog = (str: string) => console.log(`%c[Caller] ${str}`, 'color: violet');
...
function CustomUI() {
  const [callerState, setCallerState] = useState<CallerState>({
    logined: false,
    calloutState: CalloutState.NOT_START,
    calloutUser: null,
    callinUser: null,
  });
  ...
  const handleHangUser = (user: CallUser) => {
    clog(`hanged user: ${user.phone}`);
    netCall!.hangup();
  };

  const handleRefuseUser = (user: CallUser) => {
    clog(`refuse user: ${user.phone}`);
    netCall!.refuse();
  };

  const handleAnswerUser = (user: CallUser) => {
    clog(`answer user: ${user.phone}`);
    netCall!.pickup();
  };
  ...
  useEffect(() => {
    ...
    netCall = new TencentNetCall({
      ...
      onLoginStateChange: (state) => {
        setCallerState((old) => ({ ...old, logined: state.logined }));
        log(`login state change to ${JSON.stringify(state)}`);
      },
      onCallinStateChange: (state) => {
        log(`callin state change to ${JSON.stringify(state)}`);
        setCallerState((old) => {
          const newState: typeof old = { ...old };
          if (state.callinUser !== null) {
            newState.callinUser = {
              name: state.callinUser.calleeName,
              phone: state.callinUser.calleePhoneNum,
            };
          } else {
            newState.callinUser = null;
          }
          return newState;
        });
      },
      onCalloutStateChange: (state) => {
        ...
        setCallerState((old) => {
          const newState: typeof old = { ...old, calloutState: state.calloutState }
          if (state.calloutUser !== null) {
            newState.calloutUser = {
              name: state.calloutUser.calleeName,
              phone: state.calloutUser.calleePhoneNum,
            };
          }
          return newState;
        });
  ...
  useEffect(() => {
    netCall!.init();
    const sdkUiContainer = document.querySelector('.layout .login');
    sdkUiContainer?.setAttribute('style', 'display: none;');
    return () => {
      sdkUiContainer?.removeAttribute('style');
    };
  }, []);
        ...
        <Row gutter={16}>
          ...
          <Col span={7}>
            <Caller
              ...
              data={callerState}
              hang={handleHangUser}
              answer={handleAnswerUser}
              refuse={handleRefuseUser}
              check={handleCheck}
              logout={handleLogout}
            />
          </Col>
